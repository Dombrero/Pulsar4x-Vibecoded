...
namespace Pulsar4X.Engine
{
    ...
    public class MasterTimePulse : IEquatable<MasterTimePulse>
    {
        ...
        /// <summary>True while <see cref="SimulateTimeUntil"/> is calculating a TickLength increment.</summary>
        [JsonIgnore]
        public bool IsProcessingTick { get; private set; }

        /// <summary>Wall-clock UTC when the current increment calculation started.</summary>
        [JsonIgnore]
        public DateTime TickProcessStartedUtc { get; private set; }

        [JsonIgnore]
        private DateTime _tickStartGameDate;

        [JsonIgnore]
        private DateTime _tickTargetGameDate;

        /// <summary>
        /// 0–1 how far the current TickLength has advanced by game date (useful when interrupts fire).
        /// </summary>
        [JsonIgnore]
        public double TickProgress
        {
            get
            {
                if (!IsProcessingTick)
                    return 0;
                long total = (_tickTargetGameDate - _tickStartGameDate).Ticks;
                if (total <= 0)
                    return 1;
                return Math.Clamp((double)(GameGlobalDateTime - _tickStartGameDate).Ticks / total, 0.0, 1.0);
            }
        }

        /// <summary>Fired when an increment calculation starts (lightweight TimeChanged for the HUD bar).</summary>
        public event Action? TickStarted;

        /// <summary>Fired when an increment calculation finishes.</summary>
        ...
        /// <summary>
        /// Continuous time simulation: realtime delay (<see cref="TickFrequency"/>), then calculate one
        /// <see cref="Ticklength"/> increment, repeat.
        /// </summary>
        ...
        private void SimulateTimeUntil(DateTime targetDateTime, CancellationToken ct = default)
        {
            ...
            TickProcessStartedUtc = DateTime.UtcNow;
            _tickStartGameDate = GameGlobalDateTime;
            _tickTargetGameDate = targetDateTime;
            TickStarted?.Invoke();
            ...
            try
            {
                ...
                while (GameGlobalDateTime < targetDateTime && !ct.IsCancellationRequested)
                {
                    ...
                    {
                        int dop = Math.Max(1, Environment.ProcessorCount - 1);
                        Parallel.ForEach(activeSystems, new ParallelOptions { MaxDegreeOfParallelism = dop }, starSys =>
                        {
                            try
                            {
