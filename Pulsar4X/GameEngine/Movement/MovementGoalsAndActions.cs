...
public class MoveToPlan : IGoalToActionsPlanner
{
    ...
    {
        var empty = new List<EntityCommand>();

        if (ship.Manager == null || !ship.Manager.TryGetGlobalEntityById(goal.TargetEntityID, out Entity requested))
    ...
    static List<EntityCommand> Fail(Goal goal, string message)
    {
        goal.Status = GoalStatus.Failed;
...
/// <see cref="WarpMoveCommand"/> and so had already decided to warp before asking whether warp
...
public static class MovePlanner
{
    ...
    static MoveOption EvaluateWarp(Entity ship, Entity target, DateTime now)
    {
        ...
        // the up-front cost is knowable here. WarpMoveCommand holds the goal on "Charging
    ...
    public static bool TryBuildMoveActions(Entity ship, int targetId,
                                           out List<EntityCommand> actions, out string reason)
    {
        actions = new List<EntityCommand>();
        reason = string.Empty;

        if (ship.Manager == null || !ship.Manager.TryGetGlobalEntityById(targetId, out var requested))
        {
    ...
    /// </summary>
    internal static List<EntityCommand> BuildActions(Entity ship, Entity target, DateTime now, MoveOption chosen)
    {
        var actions = new List<EntityCommand>();
        ...
        switch (chosen.Mode)
        {
            ...
            case MoveMode.Warp:
            {
                // CreateCommandEZ already bundles transit + optional circularise for legacy callers.
                // Goals path uses the same plot so existing warp physics stay unchanged.
                actions.Add(WarpMoveCommand.CreateCommandEZ(ship, target, now));
                break;
            }
    ...
    internal static List<EntityCommand> BuildWarpAndCircularise(Entity orderEntity, Entity targetEntity, DateTime now)
        => new List<EntityCommand> { WarpMoveCommand.CreateCommandEZ(orderEntity, targetEntity, now) };
    
    /// <summary>
    /// Prograde-frame burns become NewtonSimpleCommands: each carries the orbit it starts from and
    ...
    /// </summary>
    static List<EntityCommand> BuildBurns(Entity ship, DateTime now, (Vector3 deltaV, double timeInSeconds)[] manuvers)
    {
        var actions = new List<EntityCommand>();
        ...
        foreach (var manuver in manuvers)
        {
            ...
            actions.Add(NewtonSimpleCommand.CreateCommand(ship.FactionOwnerID, ship, nodeTime, startKE, endKE));
